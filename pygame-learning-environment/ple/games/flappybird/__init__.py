...
class BirdPlayer(pygame.sprite.Sprite):
    ...
    def __init__(self,
                 ...
                 image_assets, rng, jump_vel, gravity, color="red",):
    ...
    def update(self, dt):
        ...
        if (self.game_tick + 1) % 15 == 0:
            ...
            self.image = self.image_assets[self.color][self.current_image]
            #self.image = pygame.transform.scale(self.image, (int(self.image.get_width()*2),  <-- THIS CHANGES THE IMAGE SCALE
...
class Target():
    """
    This is the target line that at each time the bird drops below this target
    ...
    """
    def __init__(self, target_distance, pipe_gap, color=(255,0,0), line_size=125):
        self.target_distance = target_distance   # target distance above pipes
        self.targets = []
        self.color = color
    ...
    def update(self, pipes):
        self.targets = []
        for p in pipes:
            self.targets.append((p.x, (p.gap_start + self.pipe_gap)-self.target_distance))

    def draw(self,screen):
        for t in self.targets:
            pygame.draw.line(screen, self.color, (t[0]-(self.LINE_SIZE/2),t[1]), (t[0]+(self.LINE_SIZE/2),t[1]), 2)
...
class FlappyBird(base.PyGameWrapper):
    ...
    width : int (default: 288)
        Screen width. Consistent gameplay is not promised for different widths or heights,
        therefore the width and height should not be altered.
    ...
    def __init__(self, settings):
        ...
        base.PyGameWrapper.__init__(self, settings.world_width, settings.world_height, actions=actions)
        ...
        self.target_group = Target(self.pipe_gap/2, self.pipe_gap)      # agent will try to stay above this line
        self.backdrop = None
        self.player = None
    ...
    def init(self):
        ...
        if self.player is None:
            self.player = BirdPlayer(
                ...
                self.images["player"],
                self.rng,
                jump_vel=self.settings.jump_vel,
    ...
    def getGameState(self):
        ...
        for p in self.pipe_group:
            if p.x > (self.player.pos_x - p.width/3.0):
