...
A high~\gls{ci} is associated with an intense interaction because a wide openness of the body is interpreted as a higher focus level of the player w.r.t the robot and the game; for this reason it will boost the final outcome. Low~\gls{ci}, in turn, is interpreted as a lack of focus or, in general, limited motivation of the player, which penalizes the interaction.%TODO: Not providing data and analysis to support this makes this assumption quite weak.
...
where $\kappa= (1-\rho)c$. The constant $c$ is needed to modify the range of the variable and it is greater than zero. When the estimated interaction $\rho$ is zero (meaning no interaction) the activity level is only $\alpha$. %TODO Actually, it is $\alpha$*c... Why 1-$rho$?
