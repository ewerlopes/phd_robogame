...
\input{chart/hardware_map.tex} %TODO Planning is completely missing in the chart as well as in the description. Even if it is basic it deserves a subsection, and a ball here.
...
%TODO This section is irrelevant for your work
...
Another aspect that had to be addressed was that of guaranteeing the decoupling between robot rotations and translations. %TODO Explain why. The relationship between this and slippage is not clear. Moreover the problem was due to the need to keep the person tracked with Kinect and this has to be mentioned somehow.
...
%TODO Here there are some problems. The two figures have the same caption In the present compilation of the pdf figures are somehow not included (I can see only file names).
...
\begin{figure}[H]
	...
	\caption{Membership functions (MMFs) for minimum detected distance \textit{index} in each sensing sector.} %TODO Maybe "direction" instead of "minimum detected distance"? It is not clear what is "index".
	\label{indexesmmf} 
\end{figure}
\begin{figure}[H]
	\centering
...
Since the ultimate goal of the robot is to physically tear down one of the four towers that are placed in the playground, the fuzzy rules also implement an attractive effect (figure\ref{avoid2}) that is exerted by the target tower when the robot comes close to it  ($near\_goal == true$, again refer to algorithm~\ref{alg:pointopoint}). %TODO It is not clear how this last matches the description of the navigation given before that seemed to bring the robot to the tower. Maybe you may say there that if the obstacle is a tower, the fuzzy algorithm will bring the robot against it, so that when the reader comes here can get no surprise.
...
% \textcolor{red}{Put the new tracker here}