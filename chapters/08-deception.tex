\chapter{Supporting entertainment using social interaction analysis and deception in RoboTower 2.0}\label{ch:deception}
...
$\alpha = \alpha + \frac{\overrightarrow{t_{robot}}^{(i)}}{\sum_{i}{\overrightarrow{t_{robot}}^{(i)}}} \cdot \frac{\Delta}{total}$
}
...
%TODO Here, I would start another subsection stating first what is intended to be a deceptive action in our game, how it is expected to be perceived by the player, why, and then presenting how it is obtained.
...
%TODO This may be skipped here, if you put the start of the section above \subsection{Communicating false goals}\label{sec:deception_communicate}
%Having presented how to trigger deception, in this section we describe how it is possible to use deceptive behavior by communicating false goals, i.e., false attempts to knock down towers. We begin by describing the two methods used.
...
The algorithm can choose between two different types of trajectories. The first one is activated when the robot is halfway through %TODO What does this mean? This reference does not make sense. Please give it w.r.t. the field.
...
The trajectories are implemented by deciding a series of points to be followed. These points are spatially distributed along the desired trajectory (Figure \ref{fig::trajectoryStatic}).
...
%TODO Perche' queste sono state commentate? Non ci sono? Sono fatte in modo diverso? Dire come sono generate le traiettorie mi sembra sia sensato.
...
%[EWERTON] --> \subsubsection{A machine learning approach} {\color{red} This is going to be used in a follow up project. What about some Journal? YES Social Robotics. Lets' write it with the proper introduction, taking the material that is here, to say that deceptive behavior should be designed also as an action for what is perceived by the player, not only for what is needed to trigger it.}
...
%TODO I would not say this, which is irrelevant here ----> Speed and acceleration values define two different levels: easy (speed = $0.5m/sec$ and acceleration = $0.1m/sec^2$) and medium (speed = $0.7m/sec$ and acceleration = $0.5m/sec^2$).
%However, in the trials we have noticed that the easy setting turned out to be too slow. Therefore, in order to maintain consistency in the analysis, while also improving fun we have decided to keep the difficulty fixed at the medium level for all the subjects. 
...
After every match, a questionnaire was administered to each player. It included few questions about the subject (including age and gender) and eleven questions about the game. In this paper, we present results about only four of them, relevant to evaluate the specific hypotheses. %TODO Either put all of them in the thesis, or say that there were only 4 questions.
...
%TODO Put here also results showing that H5 has to be rejected, which would support the position that deceptive behavior should be well designed, and perceived as such.
