...
Following this, we investigate the relation between the dimensions of \textit{progress} and \textit{effort}. The former relates to the quantification of the performance a given player is likely to have when facing a given difficulty level. %TODO Wouldn't be "performance" more direct than "progress". EWERTON: Progress is more directly related to score, no? ANDY "Progress" is giving an idea of dynamic advancement, change, while in this case it is more the score one is expected to achieve, which is a measure of the present performance. I've fixed it as you can see.
...
% EWERTON: Without number it is difficult to say what the contributions of this chapter really are. In words, I propose to use the recommendation system framework for suggesting "optimal" behavior parameters to players, adapting to them. With respect to CTR, CTE inverts the role of LDA. Instead of this model being used in the item latent variables, it is used on the player (I think the text says that). This way, I hypothesize that having a description of the actions (which is related to effort, in this case) would help to find the appropriate ds in less time. Note that LDA is primarily used to "group" their object of manipulation (text or players as in our case), thus, by grouping players coherently we can generalize well in the choice of ds for similar players exploiting collaborative filtering. ANDY What you write in this comment would be fine to give relevance o the chapter. OK, you do not have numbers, but the idea seems to be fine, and these motivations make it strong enough to be mentioned in your thesis.
