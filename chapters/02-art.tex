...
%TODO I do not understand why you have to mention the above partition: navigation requires all the mentioned functionalities, as well. Either make it consistent or just drop it.
...
In the dimension of playing for competitive advantage, opponent tracking is a required component since it is necessary to monitor the player activity and act accordingly. As it is going to be exposed later, the existence of an explicit model for the player activity in this dimension is optional %TODO See comment below
, since it is possible to design agents that are able to effectively play while maintaining only a minimal, general (non game-related) model of the player, such as mathematical position filtering models using bayesian filters (\eg Adaptive Monte Carlo Methods, Kalman Filter, etc). As an example, the player tracking model we have used in our experiments~\ref{sec:player_tracking} is a generic mathematical model which estimates the player's position as a particle in space. %TODO Nice to write this, but it is not in contrast with "player tracking", which can be done in many ways, but it does not seem optional. What you do is actually tracking, and I cannot see any PIRG where tracking could not be needed.

Player models that try to picture players from their in-game actions and attributes can also be considered in this phase of development. Such player models would keep a record of the player behavior and use that to take advantage and increase the chances of winning the game. %It has a different purpose of player behavior modeling for entertainment maximization, as it will be said next
%Player modeling to support competition can be viewed as a base component to modeling for entertainment maximization. This last is the central point of interest in~\gls{pirg} applications. %TODO Anticipating it here is confusing.
