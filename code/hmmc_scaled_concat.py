...
class HMM:
    ...
    def fit(self, X, max_iter=25, eps=10e-2, verbose=False):
    ...
    def generate_samples(self, N=20, T=100):
        assert(np.all(self.A <= 1))
        X = []  # the data, the N x T
        for i,n in enumerate(xrange(N)):
            x = np.zeros((T, self.mu.shape[-1]))
            ...
            #   on the first stage.)
            r = np.random.choice(self.K, p=self.R[s])  # choose mixture
            x[0] = np.random.multivariate_normal(self.mu[s,r], self.sigma[s, r])
            for t in xrange(1, T):
                s = np.random.choice(self.M, p=self.A[s]/np.sum(self.A[s]))  # choose state
                r = np.random.choice(self.K, p=self.R[s])  # choose mixture
                x[t] = np.random.multivariate_normal(self.mu[s, r], self.sigma[s, r])  # Return a D-dimensional array
            X.append(x)
        return X
...
def fake_signal(init=big_init):
    T = 128
    signals = get_signals(N=1, T=T,init=init)
    ...
    actual_hmm = HMM(5, 3)
    actual_hmm.set(pi, A, R, mu, sigma)
    L = actual_hmm.log_likelihood_multi(signals).sum()
    print "LL for actual params:", L

    est_samples = hmm.generate_samples(1, T)
