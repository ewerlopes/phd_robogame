...
class MeanShift(object):
    ...
    def cluster(self, points, kernel_bandwidth, iteration_callback = None, **kwargs):
        ...
        while max_min_dist > MIN_DISTANCE:
            ...
            for i in range(0, len(shift_points)):
                ...
                p_new_start = p_new
                p_new = self._shift_point(p_new, points, kernel_bandwidth, **kwargs)  
    ...
    def _shift_point(self, point, points, kernel_bandwidth, **kwargs):
        # from http://en.wikipedia.org/wiki/Mean-shift
        points = np.array(points)
        # numerator
        point_distances = np.sqrt(((point - points)**2).sum(axis=1))
        point_weights = self.kernel(point_distances, kernel_bandwidth, **kwargs)    
