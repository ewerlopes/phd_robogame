...
if __name__ == "__main__":
    ...
    print 65 * '*'
    print
    print "CSV dir: \t{}".format(args.csv_dir)
    print "N.Files: \t{}".format(len(files))
    print "Ref. overlap: \t{}%".format(args.deviation)
    print "N. Windows (Avg): \t{:.2f}".format(np.mean(total_windows))
    print "Overlap    (Avg): \t{:.2f}%".format(np.mean(total_overlap))
    print "Overlap    (Std) : \t{:.2f}%".format(np.std(total_overlap))
    print "Overpap dev. from ref. (Avg): \t{:.2f}%".format(np.mean(total_diff))
    print "Overpap dev. from ref. (Std): \t{:.2f}%".format(np.std(total_diff))
    print
    for k in csv_data.keys():
