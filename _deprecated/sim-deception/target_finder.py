...
def distance(point1, point2):
    dist = numpy.sqrt((point1[0] - point2[0]) ** 2 + (point1[1] - point2[1]) ** 2)
    return dist
...
def check_for_negative_values(target_preferences):
    ...
    if len(negative_values) > 0:
        max_neg = numpy.max(negative_values)
        for i in range(4):
...
def target_preferences_robot(player_xy, robot_xy, towers_xy, previous_target):
    ...
    # wrp = math.degrees(math.atan2(player_xy[1] - robot_xy[1], player_xy[0] - robot_xy[0])) + 180
    # wrt = ([0.0, 0.0, 0.0, 0.0])
    target_preferences = ([0.0, 0.0, 0.0, 0.0])

    for i in range(4):
        # wrt[i] = math.degrees(math.atan2(towers_xy[i][1] - robot_xy[1], towers_xy[i][0] - robot_xy[0])) + 180
