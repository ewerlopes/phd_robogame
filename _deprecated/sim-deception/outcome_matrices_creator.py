...
def outcome_matrix_robot(target_array, towers_xy, player_xy, robot_xy):
    ...
    for column in range(4):
        for row in range(4):
            if row == column:
                outcome_matrix[row][column] =  target_finder.distance(towers_xy[row, :], robot_xy) / 100 #target_array[column] *
            else:
                outcome_matrix[row][column] = 10000 * (
...
def outcome_matrix_player(target_array, towers_xy, robot_xy):
    ...
    for row in range(4):
        for column in range(4):
            if row == column:
                outcome_matrix[row][column] = 10000 * 1 / target_finder.distance(towers_xy[column, :], robot_xy)
            else:
                outcome_matrix[row][column] = 100 * (target_array[row] / numpy.sum(target_array) * target_finder.distance(towers_xy[column, :], robot_xy) / sum_distances_robot_towers)
